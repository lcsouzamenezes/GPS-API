...
class Service extends REST_Controller
{
                 ...
                 if($phonenumber){
                   ...
                   // $ins_data['plan_id']        = $plan_id;
                    ...
                    if(!empty($default_id)) {
                     ...
                     if(!empty($phonenumber) && ($logintype!='website')) {
   ...
   function search_map_get($join_key = '', $user_id ='', $type='')
   {
      ...
      // echo count($result); exit;
       ...
       // if(empty($uct)){
//            if(isset($plan_data['members_count']) && ($plan_data['members_count']==0) && ($user_data['login_type'] == 'app') ) {
//                return $this->response(array('status' => "error",'msg' => "You can't access. Your plan has been expired!",'error_code' => 1), 404);
//                continue;
//            }
//        }
//        
            ...
            if(empty($cnt) && (int)$user_id) {
                 ...
                 if($user_data['login_type'] != 'website'){
                    ...
                    //if($user_id != $result['user_id']) {
                        //$in_member_count = $plan_data['members_count']-1;
                        //update_plan_data($result['user_id'],$in_member_count);
                   // } 
                 } 
              }
            //get user data
            ...
            if(count($user_details)){
                ...
                $lastseenup='';
                     ...
                     if(count($position)) {
                         ...
                         {
                            //  echo $result['user_id']." ".$pvalue['show_personal'].$pvalue['status'];
                              ...
                              // $user[$i]['user']['group']['last_seen_time']  = strtotime($pvalue['last_seen_time']);
                                $user[$i]['user']['group']['last_seen_time'] = $pvalue['lastseen'];
                               $user[$i]['user']['group']['is_online']       = $lastup;
                               $user[$i]['user']['group']['share_link']      = site_url()."search/".$pvalue['group_join_key'];
    ...
    function allowdeny_send_notification_get()
    {
        ...
        if(count($admin)){
            ...
            if(!empty($user_id) && !empty($gcm_id)) {  
                ...
                $notification_staus =  $this->gcm->send_notification(array($gcm_id),array("hmg" => $gcm_data));
