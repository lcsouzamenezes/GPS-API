...
class User extends Admin_controller {
    ...
    function send_notification()
    {
       ...
       // $gcm_data['title']       = $form['title'];
        $gcm_data['message']       = $_POST['message']; 
       // $gcm_data['description'] = $form['description'];
        
        //gcm_id
        $user   = $this->user_model->check_unique(array("id" => $_POST['notify_user_id']));
        $gcm_id = $user['gcm_id'];
        $this->gcm->send_notification(array($gcm_id),array("hmg" => $gcm_data));
        
        $this->db->query("insert into admin_notifications set user_id='".$_POST['notify_user_id']."', message='".$_POST['message']."', created_date='".date("Y-m-d H:i:s")."'");
       
       if($this->input->is_ajax_request())
         return    $this->_ajax_output(array('status' => 'success','gcm_id' => $gcm_id ,'msg' => 'Notificaton Send Successfully!' ), TRUE);
    ...
    function admin_send_mail()
    {
        ...
        $message = (!empty($_POST['message']))?$_POST['message']:"";
         ...
         $this->email->from('jeff@holdenusa.com','Jeff');
         $this->email->to($email);
         ...
         if($this->email->send()){
            if($this->input->is_ajax_request())
                return   $this->_ajax_output(array('status' => 'success' ,'msg' => 'Mail Sent Successfully!'), TRUE);
         }
         else
         {
            return   $this->_ajax_output(array('status' => 'error' ,'msg' => 'There is error in sending mail!'), False);
         }   
