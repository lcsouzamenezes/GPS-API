...
class REST_Controller extends CI_Controller {
	...
	public function __construct()
	{
		...
		{
			$this->response( array('status' => 'error', 'msg' => 'Only AJAX requests are accepted.','error_code' => 505), 505 );
		}
	}
	...
	public function _remap($object_called, $arguments)
	{
		...
		{
			$this->response(array('status' => 'error', 'msg' => 'Unknown method.','error_code' => 7), 404);
		}
		...
		if (config_item('rest_enable_keys') AND ! empty($this->rest->key))
		{
			...
			{
				$this->response(array('status' => 'error', 'msg' => 'This API key has reached the hourly limit for this method.','error_code' => 8), 401);
			}
			...
			$authorized OR $this->response(array('status' => 'error', 'msg' => 'This API key does not have enough permissions.','error_code' => 9), 401);
		}
	...
	public function response($data = array(), $http_code = null)
	{
	  //print_r($data); exit;
		...
		else
		{
			...
			{
				//
				// to display json content and not to give as a download
		...
		// if condition to display json content and not to give as a download		
		if($this->response->format != 'json')
		{
...
//           }          
	...
	protected function _check_whitelist_auth()
	{
		...
		{
			$this->response(array('status' => 'error', 'msg' => 'Not authorized','error_code' => 401), 401);
		}
	}
	...
	protected function _force_login($nonce = '')
	{
		...
		$this->response(array('status' => 'error', 'msg' => 'Not authorized','error_code' => 401), 401);exit();
	}
